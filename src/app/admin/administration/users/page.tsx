...
export default async function UsersAdminPage(){
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="font-display text-2xl font-light">Users</h1><p className="text-sm text-muted-foreground">{data?.length||0} staff • {myRole} • HOTEL_ADMIN can CRUD except SUPER_ADMIN • current: {currentEmail}</p></div><a href="/admin/administration/users/new" className="h-9 px-4 bg-brand-foreground text-brand-background inline-flex items-center text-xs tracking-widest hover:bg-brand-foreground/90">+ NEW USER</a></div>
  ...
  <tbody>{(data||[]).map(u=> {
    ...
    return <tr key={u.id} className={"border-b last:border-0 hover:bg-muted/30 "+(isMe?'bg-brand-accent/10':'')}><td className="py-3 font-medium">{u.first_name} {u.last_name} {isMe&&<span className="text-[10px] bg-brand-accent text-white px-1 rounded">YOU</span>}</td><td className="text-xs font-mono">{u.email}</td><td><Badge variant={u.role==='SUPER_ADMIN'?'destructive': u.role==='HOTEL_ADMIN'?'default':'secondary'}>{u.role}</Badge></td><td className="text-xs font-mono">•••••••• <span className="text-[10px] text-muted-foreground">(demo: 121212)</span></td><td className="text-right flex justify-end gap-2 items-center"><ChangePasswordDialog email={u.email} role={u.role} disabled={disabled} disabledReason={disabled ? `Tidak boleh ubah ${u.role} sebagai ${myRole}` : undefined} /><a href="/admin/administration/users" className="h-7 px-3 border border-border grid place-items-center text-xs hover:bg-muted">EDIT</a><a href="/admin/administration/users" className="h-7 px-3 border border-border grid place-items-center text-xs text-destructive hover:bg-destructive/10">DELETE</a></td></tr>
