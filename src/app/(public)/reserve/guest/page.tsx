...
export default async function GuestPage({ searchParams }: { searchParams: Promise<{ room_type?: string, check_in?: string, check_out?: string, guests?: string, nights?: string, rate?: string }> }) {
  ...
  return (
    <div className="container mx-auto px-6 py-12 max-w-3xl">
      ...
      <p className="text-sm text-muted-foreground mb-8">Kamar: <LocalizedText value={roomType?.name as any} fallback="Kamar Pilihan" /> • {sp.check_in} → {sp.check_out} • {sp.guests} tamu • <Price amount={rate} showRate /> • Subtotal <Price amount={rate * nights} /></p>
        ...
        <CardContent>
          <form action={`/reserve/review?room_type=${sp.room_type}&check_in=${sp.check_in}&check_out=${sp.check_out}&guests=${sp.guests}&nights=${sp.nights}&rate=${sp.rate}`} method="GET" className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2"><Label>{tr('booking.first_name')} *</Label><Input name="first_name" defaultValue={guest?.first_name || (user?.user_metadata as any)?.first_name || ''} required /></div>
              <div className="space-y-2"><Label>{tr('booking.last_name')} *</Label><Input name="last_name" defaultValue={guest?.last_name || (user?.user_metadata as any)?.last_name || ''} required /></div>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2"><Label>{tr('booking.email')} *</Label><Input name="email" type="email" defaultValue={guest?.email || user?.email || ''} required /></div>
              <div className="space-y-2"><Label>{tr('booking.phone')}</Label><Input name="phone" defaultValue={guest?.phone || ''} /></div>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2"><Label>{tr('booking.city')}</Label><Input name="city" defaultValue={guest?.city || ''} /></div>
              <div className="space-y-2"><Label>{tr('booking.country')}</Label><Input name="country" defaultValue={guest?.country || ''} /></div>
            </div>
