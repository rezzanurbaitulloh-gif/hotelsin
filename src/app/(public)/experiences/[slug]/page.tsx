export default function Page({ params }: { params: { slug?: string; id?: string } }) {
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-6 py-24 text-center">
      ...
      <h1 className="font-display text-4xl md:text-5xl font-light tracking-tight mb-4">Experience</h1>
      <p className="text-xs tracking-widest text-muted-foreground uppercase mb-2">Slug / ID: {params.slug ?? params.id ?? '—'}</p>
      <p className="text-muted-foreground max-w-xl leading-relaxed mb-8">Experience detail with duration, pricing and inclusions.</p>
      ...
      <p className="mt-8 text-xs text-muted-foreground">Route: <code className="bg-muted px-2 py-1 rounded">(public)/experiences/[slug]</code> — rendering OK (zero-404 guarantee)</p>
    </div>
  )
}
